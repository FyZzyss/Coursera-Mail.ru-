...
class Client:
    ...
    def put(self, key, value, timestamp=None):
        ...
        print(f"COMMAND: put {key} {value} {timestamp}\n".encode())
        raw_data = self._read()
        print(raw_data)

        if raw_data == 'ok\n\n':
            return
        raise ClientError('Server returns an error' + raw_data)

    def get(self, key):
        self._send(f"get {key}\n".encode())
        print(f"COMMAND: get {key}\n".encode())
        raw_data = self._read()
        print(raw_data)
        data = {}
        ...
        if status != 'ok':
            raise ClientError('Server returns an error' + payload)
